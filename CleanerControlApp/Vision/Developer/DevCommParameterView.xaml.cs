...
namespace CleanerControlApp.Vision.Developer
{
    ...
    public partial class DevCommParameterView : UserControl
    {
        ...
        // Default constructor: attempt to resolve IServiceProvider from App host, then initialize
        public DevCommParameterView()
        {
            // Try to obtain IServiceProvider from the App host (if app is started via Host)
            try
            {
            ...
            // Attempt to get CommunicationSettings from DI; fall back to ConfigLoader or default
            CommunicationSettings? commFromDi = null;
            try
            {
                if (serviceProvider != null)
                {
                    commFromDi = serviceProvider.GetService(typeof(CommunicationSettings)) as CommunicationSettings;
                }
            }
            catch { commFromDi = null; }

            if (commFromDi != null)
                _commSettings = commFromDi;
            else
            {
                // Try ConfigLoader at runtime (safe if app host not available)
                try
                {
                    _commSettings = ConfigLoader.GetCommunicationSettings() ?? new CommunicationSettings();
                }
                catch
                {
                    _commSettings = new CommunicationSettings();
                }
            }

            // Initialize UI and populate controls
            InitializeComponent();
        ...
        // Constructor that accepts IServiceProvider and obtains CommunicationSettings from DI if available
        public DevCommParameterView(IServiceProvider serviceProvider) : this(serviceProvider?.GetService(typeof(CommunicationSettings)) as CommunicationSettings ?? new CommunicationSettings())
        {
            this.serviceProvider = serviceProvider;
        }

        // Constructor that accepts both CommunicationSettings and IServiceProvider
        public DevCommParameterView(CommunicationSettings commSettings, IServiceProvider serviceProvider) : this(commSettings)
        {
            this.serviceProvider = serviceProvider;
        }
        ...
        private void BtnLoadParams_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            try
            {
                // Prefer settings from DI/service provider; fall back to existing in-memory settings or default
                var fromDi = serviceProvider?.GetService(typeof(CommunicationSettings)) as CommunicationSettings;
                if (fromDi != null)
                {
                    _commSettings = fromDi;
                }
                else if (_commSettings == null)
                {
                    _commSettings = new CommunicationSettings();
                }

                // Refresh UI from the in-memory / DI-provided communication settings
                if (_commSettings != null)
                {
                    LoadModbusTcpParametersFromSettings();
                    LoadModbusRtuParameters();
                    MessageBox.Show("把计眖莱ノ祘Α既砞﹚弄穝礶", "弄把计", MessageBoxButton.OK, MessageBoxImage.Information);
                }
                else
                {
                    MessageBox.Show("ゼ矗ㄑ既砞﹚礚猭穝礶", "弄把计", MessageBoxButton.OK, MessageBoxImage.Warning);
                }
            }
            catch (System.Exception ex)
            {
                MessageBox.Show("弄把计ア毖: " + ex.Message, "岿粇", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void BtnSaveParams_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            try
            {
                // Update _commSettings from UI
                UpdateSettingsFromUI();

                // Persist to appsettings.json using ConfigLoader
                try
                {
                    if (_commSettings != null)
                    {
                        ConfigLoader.SetCommunicationSettings(_commSettings);
                        MessageBox.Show("把计糶砞﹚郎 (appsettings.json)", "糶把计", MessageBoxButton.OK, MessageBoxImage.Information);
                    }
                }
                catch (System.Exception ex)
                {
                    MessageBox.Show("糶砞﹚郎ア毖: " + ex.Message, "岿粇", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
            catch (System.Exception ex)
            {
        ...
        // New button handler: write parameters into in-memory settings (do not persist to file)
        private void BtnWriteParams_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            try
            {
                UpdateSettingsFromUI();
                MessageBox.Show("把计糶莱ノ祘Α既砞﹚ (ゼ糶郎)", "糶把计", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (System.Exception ex)
            {
                MessageBox.Show("糶把计ア毖: " + ex.Message, "岿粇", MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        // Helper to update _commSettings from UI controls
        ...
        private void LoadModbusRtuParameters()
        {
            ...
            try
            {
                ...
                if (comm != null)
                {
                    ...
                    string portNameVal = "COM1", baudVal = "9600", parityVal = "None", dataBitsVal = "8", stopBitVal = "1";
